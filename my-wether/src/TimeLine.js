...
import {  GetTodayHours } from "./features/WeatherApi/WeatherApiSlice";
import { useDispatch, useSelector } from "react-redux";

export default function WeatherTimeline() {
  const dispatch = useDispatch();
  const hourly = useSelector((state) => state.data.hours);
  const hourlyRes = hourly?.hoursData || []; // Ensure it's not undefined

  useEffect(() => {
    dispatch(GetTodayHours()); // Fetch the data when component mounts
  }, []);
